...
export default function BookingCard({ booking, role, onAction, onReview, onOpenChat, busy }) {
  ...
  // Owner-facing payout breakdown. The server is the source of truth; fall back
  // to deriving the split for any legacy booking saved before fees existed.
  ...
  // counterparty depends on the viewing context
  ...
  // conditional actions — must mirror the server's permission rules
  const actions = [];
  if (role === 'owner') {
