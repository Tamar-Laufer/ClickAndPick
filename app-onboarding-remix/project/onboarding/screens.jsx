/* screens.jsx — גמ"ח-נט onboarding · screen content + visual sub-components.
   Exports to window: OB = { SCREENS, Placeholder, TrustCard, ImpactGrid,
   CatChips, ItemWall, RoleCards, StarRow } */

const { useState } = React;

/* ── Marked photo placeholder (warm striped, mono label) ───────────── */
function Placeholder({ label, tall }) {
  return (
    <div className={"ob-ph" + (tall ? " ob-ph--tall" : "")} role="img" aria-label={label}>
      <div className="ob-ph-grain" />
  ...
  return (
    <div className="ob-trust">
      <div className="ob-trust-head">
        ...
        <div className="ob-trust-ring">
          <svg width="76" height="76" viewBox="0 0 76 76">
            ...
            <circle cx="38" cy="38" r={R} fill="none" stroke="var(--tone)" strokeWidth="7"
                    strokeLinecap="round" strokeDasharray={C}
                    strokeDashoffset={C * (1 - pct / 100)}
                    transform="rotate(-90 38 38)" className="ob-ring-arc" />
          </svg>
      ...
      <div className="ob-vbadges">
        <VBadge label="ת״ז מאומתת" />
        <VBadge label="טלפון מאומת" />
        <VBadge label="אימייל מאומת" />
        <VBadge label="כתובת אומתה" />
      </div>
    </div>
  );
}
    ...
    <div className="ob-wall">
      {ITEMS.map((it, i) => (
        <div key={i} className="ob-wall-card">
          ...
          <div className="ob-wall-meta">
            <span className="ob-wall-t">{it.t}</span>
            <span className="ob-wall-d">{it.d}</span>
          </div>
        </div>
      ))}
    </div>
...
/* ── Impact grid ───────────────────────────────────────────────────── */
const STATS = [
  { n: "12", l: "פריטים ששאלתם", s: "" },
  { n: "₪1,240", l: "נחסכו השנה", s: "" },
  { n: "36 ק״ג", l: "פסולת שנמנעה", s: "" },
  { n: "84 ק״ג", l: "פליטות CO₂ שנחסכו", s: "" },
];
function ImpactGrid() {
  return (
      ...
      <div className="ob-impact-grid">
        {STATS.map((s, i) => (
          <div key={i} className="ob-stat">
            <strong className="ob-stat-n">{s.n}</strong>
...
function RoleCards() {
  ...
  return (
    <div className="ob-roles">
      {roles.map((r, i) => (
        <button key={i} type="button" className={"ob-role" + (sel === i ? " ob-role--on" : "")} onClick={() => setSel(i)}>
...
const SCREENS = [
  ...
  {
    key: "trust", tone: "teal", eyebrow: "אמון", hero: true,
...
window.OB = { SCREENS, Placeholder, TrustCard, ImpactGrid, CatChips, ItemWall, RoleCards, StarRow };
